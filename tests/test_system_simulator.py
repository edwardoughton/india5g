...
def test_estimate_link_budget(base_system, setup_modulation_coding_lut,
    ...
    for receiver in actual_result:
        if receiver['id'] == 'id_0':
            ...
            assert round(actual_result[0]['noise']) == -102
            assert round(actual_result[0]['sinr']) == 1
            assert round(actual_result[0]['capacity_mbps']) == 15
            assert round(actual_result[0]['capacity_mbps_km2']) == 68
